...
> **حالة التنفيذ:** المرحلة 0 (السقالة) خلصت — `backend/` (FastAPI + SQLAlchemy + هجرة
> Alembic أولية، اختُبرت: التطبيق يقلع، الـmigration تولّد SQL صحيح بدون اتصال حقيقي
> بقاعدة بيانات بعد) و`frontend/` (Vite + React + TS، شاشتا تسجيل الدخول والمتاجر
> مبنيّتين فعلياً ومتصلتين بالـAPI، باقي الشاشات حواجز مكان مؤقتة). راجع
> `backend/README.md` و`frontend/README.md` للتشغيل. **قبل ما تكمل المرحلة 1** لازم:
> (1) تثبيت Node.js محلياً (مو مثبّت على هذا الجهاز)، (2) قاعدة بيانات Postgres فعلية
> (Docker محلي عبر `docker-compose.yml` بجذر المشروع، أو حساب Supabase مجاني تنشئه
> بنفسك) — ما قدرت أشغّل الهجرة فعلياً ولا أتحقق من الاتصال الحقيقي بدون هذا.
