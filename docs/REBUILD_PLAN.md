...
> **حالة التنفيذ:** **المرحلة 1 (الباك إند) خلصت بالكامل ومتحقق منها فعلياً ضد
> Supabase حقيقية** — خط الأنابيب كامل شغّال عبر HTTP حقيقي من الرفع للتصدير:
> رفع فاتورة → تنظيف + تصنيف تلقائي → جلسة استلام التاجر → تسوية الأصناف (مطابقة
> بالباركود/الاسم + موافقة بشرية) → دمج بالمخزون التراكمي + تعلّم فهرس الأصناف
> المشترك → تصدير ملف Alsahl. كل خطوة اختُبرت على فواتير حقيقية عبر طلبات HTTP
> فعلية (مو Swagger يدوي فقط) — راجع رسائل الكوميت بالتفصيل لكل خطوة (تشمل خللين
> حقيقيين اكتُشفوا وانصلحوا بالطريق: عتبة fuzzy matching بالتصنيف، ووحدة وزن "غم"
> المفقودة بفحص تعارض الحجم بالتسوية). التوثيق الكامل لكل نقطة API بـ`backend/README.md`.
>
> **`frontend/`:** Login وStores مبنيّتين ومتصلتين بالباك إند الحقيقي، باقي شاشات
> خط الأنابيب (المراجعة/الجلسة/التسوية/التصدير/المخزون) قيد البناء الآن (المرحلة 2).
>
> **الناقص لإكمال البورتفوليو:** بقية شاشات React (مرحلة 2)، ثم النشر الفعلي على
> Vercel/Render (مرحلة 3) — راجع §8 تحت لتفاصيل كل مرحلة.
