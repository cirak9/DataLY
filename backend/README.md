...
كل نقطة محمية بتوكن JWT (`Authorization: Bearer <token>`) إلا `/auth/login` و`/auth/register`.

```
POST   /auth/register                                  تسجيل تاجر ذاتي (اسم متجر+هاتف+كلمة مرور) → متجر + JWT
POST   /auth/login                                      تسجيل دخول (بريد أو رقم هاتف) → JWT
...
- **استيراد المخزون القديم (`POST /stores/{id}/inventory/import`) upsert لا استبدال
  كامل** — يُسأل التاجر قبل كل جلسة استلام (الواجهة تعرض خطوة اختيارية)، لأن النظام
  ما يقدر يكتشف تلقائياً إضافات مخزون يدوية صارت بدون فاتورة عبرنا. نفس منطق كشف
  الأعمدة الذكي من الأداة الأصلية (`extractors/inventory_extractor.py` — تطابق دقيق
  ثم احتواء جزئي)، مع إضافة كشف اختياري لعمودي الكمية/تكلفة الوحدة (الأصلي كان
  يتتبّع باركود/اسم/صلاحية بس). نفس باركود+صلاحية موجودين مسبقاً = استبدال بالأحدث،
  مو تكرار — قرار صريح من صاحب المشروع.
...
- **تسجيل تاجر ذاتي (`POST /auth/register`) برقم هاتف بدل بريد إلكتروني** — `users.email`
  صار اختياري (حسابات إدارية قديمة تبقى تشتغل بالبريد زي ما هي)، و`users.phone_number`
